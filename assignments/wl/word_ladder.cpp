...
std::vector<std::unordered_set<std::string>> constructHops(
    const std::string& to, const std::unordered_map<std::string, int>& depths, int minHops);
...
constructLadders(std::unordered_map<std::string, std::unordered_set<std::string>>& ladderMap,
...
bool neighbour(const std::string& word1, const std::string& word2) {
  ...
  for (auto itChar1 = word1.cbegin(), itChar2 = word2.cbegin(); itChar1 != word1.cend();
       ++itChar1, ++itChar2) {
    if ((*itChar1) != (*itChar2)) {
      // difference detected
